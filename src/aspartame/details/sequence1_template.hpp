...
[[nodiscard]] constexpr auto prepend(const ASPARTAME_IN_TYPE1(C) & in, const T &t, tag = {}) {
  return details::sequence1::prepend<ASPARTAME_IN_TYPE1(C), T, ASPARTAME_IN_TYPE1(C)>(in, t);
}

template <typename C> //
[[nodiscard]] constexpr auto head_maybe(const ASPARTAME_IN_TYPE1(C) & in, tag = {}) {
  return details::sequence1::head_maybe<ASPARTAME_IN_TYPE1(C)>(in);
}

template <typename C> //
[[nodiscard]] constexpr auto last_maybe(const ASPARTAME_IN_TYPE1(C) & in, tag = {}) {
  return details::sequence1::last_maybe<ASPARTAME_IN_TYPE1(C)>(in);
}

template <typename C> //
[[nodiscard]] constexpr auto init(const ASPARTAME_IN_TYPE1(C) & in, tag = {}) {
  return details::sequence1::init<ASPARTAME_IN_TYPE1(C), ASPARTAME_IN_TYPE1(C)>(in);
}

template <typename C> //
[[nodiscard]] constexpr auto tail(const ASPARTAME_IN_TYPE1(C) & in, tag = {}) {
  return details::sequence1::tail<ASPARTAME_IN_TYPE1(C), ASPARTAME_IN_TYPE1(C)>(in);
}

template <typename C> //
[[nodiscard]] constexpr auto at_maybe(const ASPARTAME_IN_TYPE1(C) & in, size_t idx, tag = {}) {
  return details::sequence1::at_maybe<ASPARTAME_IN_TYPE1(C)>(in, idx);
}

template <typename C> //
[[nodiscard]] constexpr auto slice(const ASPARTAME_IN_TYPE1(C) & in, size_t from_inclusive, size_t to_exclusive, tag = {}) {
...
[[nodiscard]] constexpr auto index_of_slice(const ASPARTAME_IN_TYPE1(C) & in, const Container &other, tag = {}) {
  return details::sequence1::index_of_slice<ASPARTAME_IN_TYPE1(C), Container>(in, other);
}

template <typename C, typename Container> //
[[nodiscard]] constexpr auto contains_slice(const ASPARTAME_IN_TYPE1(C) & in, const Container &other, tag = {}) {
  return details::sequence1::index_of_slice<ASPARTAME_IN_TYPE1(C), Container>(in, other) != -1;
}

template <typename C, typename T> //
[[nodiscard]] constexpr auto index_of(const ASPARTAME_IN_TYPE1(C) & in, const T &t, tag = {}) {
...
template <typename C, typename Predicate> //
[[nodiscard]] constexpr auto index_where(const ASPARTAME_IN_TYPE1(C) & in, Predicate &&predicate, tag = {}) {
...
template <typename C, typename Container> //
[[nodiscard]] constexpr auto zip(const ASPARTAME_IN_TYPE1(C) & in, const Container &other, tag = {}) {
...
[[nodiscard]] constexpr auto sort_by(const ASPARTAME_IN_TYPE1(C) & in, Select &&select, tag = {}) {
  return details::sequence1::sort_by<ASPARTAME_IN_TYPE1(C), Select, ASPARTAME_IN_TYPE1(C)>(in, select);
}

template <typename C> //
[[nodiscard]] constexpr auto split_at(const ASPARTAME_IN_TYPE1(C) & in, size_t idx, tag = {}) {
  return details::sequence1::split_at<ASPARTAME_IN_TYPE1(C), ASPARTAME_IN_TYPE1(C)>(in, idx);
}

template <typename C> //
[[nodiscard]] constexpr auto take_right(const ASPARTAME_IN_TYPE1(C) & in, size_t n, tag = {}) {
  return details::sequence1::take_right<ASPARTAME_IN_TYPE1(C), ASPARTAME_IN_TYPE1(C)>(in, n);
}

template <typename C> //
[[nodiscard]] constexpr auto drop_right(const ASPARTAME_IN_TYPE1(C) & in, size_t n, tag = {}) {
  return details::sequence1::drop_right<ASPARTAME_IN_TYPE1(C), ASPARTAME_IN_TYPE1(C)>(in, n);
}

template <typename C> //
[[nodiscard]] constexpr auto take(const ASPARTAME_IN_TYPE1(C) & in, size_t n, tag = {}) {
  return details::sequence1::take<ASPARTAME_IN_TYPE1(C), ASPARTAME_IN_TYPE1(C)>(in, n);
}

template <typename C> //
[[nodiscard]] constexpr auto drop(const ASPARTAME_IN_TYPE1(C) & in, size_t n, tag = {}) {
  return details::sequence1::drop<ASPARTAME_IN_TYPE1(C), ASPARTAME_IN_TYPE1(C)>(in, n);
}

template <typename C, typename Predicate> //
[[nodiscard]] constexpr auto take_while(const ASPARTAME_IN_TYPE1(C) & in, Predicate &&predicate, tag = {}) {
...
[[nodiscard]] constexpr auto fold_left(const ASPARTAME_IN_TYPE1(C) & in, Accumulator &&init, Function &&function, tag = {}) {
  return details::sequence1::fold_left<ASPARTAME_IN_TYPE1(C), Accumulator, Function>(in, std::forward<Accumulator &&>(init), function);
}

template <typename C, typename Accumulator, typename Function> //
[[nodiscard]] constexpr auto fold_right(const ASPARTAME_IN_TYPE1(C) & in, Accumulator &&init, Function &&function, tag = {}) {
  return details::sequence1::fold_right<ASPARTAME_IN_TYPE1(C), Accumulator, Function>(in, std::forward<Accumulator &&>(init), function);
}

template <typename C> //
[[nodiscard]] constexpr auto sliding(const ASPARTAME_IN_TYPE1(C) & in, size_t size, size_t step, tag = {}) {
