...
auto mk_string(const ASPARTAME_IN_TYPE2(K, V) & in, const std::string_view &sep, const std::string_view &prefix,
...
template <typename K, typename V, typename Predicate> //
constexpr auto find(const ASPARTAME_IN_TYPE2(K, V) & in, Predicate &&predicate) {
