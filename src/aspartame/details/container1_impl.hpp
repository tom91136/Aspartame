...
[[nodiscard]] auto mk_string(const In &o, const std::string_view &sep, const std::string_view &prefix, const std::string_view &suffix,
                             Function f) {
  static_assert(std::is_convertible_v<decltype(details::ap(f, *std::begin(o))), std::string>, "function does not return a string type");
  std::string out{prefix};
  size_t size = o.size();
  size_t idx = 0;
  for (auto x : o) {
    out += details::ap(f, x);
    if (idx < size - 1) out += sep;
    idx++;
  }
  out += suffix;
  return out;
...
[[nodiscard]] constexpr auto bind(const In &in, Function f) {
  static_assert(is_iterable<decltype(details::ap(f, *std::begin(in)))>, "bind function should return an iterable type");
  using T = typename decltype(details::ap(f, *std::begin(in)))::value_type;
  Out<T> ys;
  for (auto it = in.begin(), end = in.end(); it != end; ++it) {
    auto x = *it;
...
[[nodiscard]] constexpr auto distinct_by(const In &in, Function f) {
  using T = typename In::value_type;
  using V = decltype(details::ap(f, *std::begin(in)));
  ...
  if (in.size() == 1) return Out{*std::begin(in)};
...
[[nodiscard]] constexpr In tap_each(const In &in, Function f) {
  if constexpr (details::assert_void<decltype(details::ap(f, *std::begin(in)))>()) {}
  for (auto it = in.begin(), end = in.end(); it != end; ++it) {
    auto x = *it;
    details::ap(f, x);
  }
