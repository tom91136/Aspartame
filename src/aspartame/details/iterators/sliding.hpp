...
//      window.clear();
  ...
  [[nodiscard]] constexpr const W &operator*() {
    return std::move(W{window_buffer.begin(), window_buffer.begin() + std::min(window_buffer.size(), size)});
  }

  [[nodiscard]] constexpr bool operator==(const sliding_iterator &that) const {
    //      return it == that.it && window_buffer == that.window_buffer;
