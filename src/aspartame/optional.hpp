...
template <typename C, typename Function> //
[[nodiscard]] constexpr auto bind(const std::optional<C> &in, Function &&function, tag={}) {
...
template <typename C, typename Predicate> //
[[nodiscard]] constexpr auto partition(const std::optional<C> &in, Predicate &&predicate, tag={}) -> std::pair<std::optional<C>, std::optional<C>> {
  if constexpr (details::assert_predicate<decltype(details::ap(predicate, *in))>()) {}
...
[[nodiscard]] constexpr auto group_map_reduce(const std::optional<C> &in, GroupFunction &&group, MapFunction &&map, ReduceFunction &&, tag={}) {
