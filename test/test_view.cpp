...
//        std::for_each(actual.begin(), actual.end(), expected.begin(), [&](auto &&view, auto &&expected) {
//          std::vector actual_vector{view.begin(), view.end()};
//          if (invert) CHECK_FALSE(actual_vector == expected);
//          else CHECK(actual_vector == expected);
//        });

      } else {
        std::vector<T> actual_vector{actual.begin(), actual.end()};
        if (invert) CHECK_FALSE(actual_vector == expected);
        else CHECK(actual_vector == expected);
      }
    };
    ...
    if constexpr (aspartame::is_pair<R>) {
      if constexpr (aspartame::is_view<typename R::first_type> && aspartame::is_view<typename R::second_type>) {
        ...
        check_direct(f(v), expected);
      }
    } else if constexpr (aspartame::is_view<R>) {
      check_view(f(v), expected);
    } else {
      check_direct(f(v), expected);
    }
